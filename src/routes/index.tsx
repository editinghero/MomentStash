      ...
      <ul className="hidden md:flex items-center gap-8">
        {links.map((l, i) => (
          <li key={l.label} className="relative">
            <Link
              to={l.to}
              className="font-accent text-sm font-semibold text-ink hover:text-primary transition-colors"
            >
              {l.label}
            </Link>
            {i === 0 && (
...
function Hero() {
  return (
    <section className="relative mx-auto max-w-7xl px-6 pt-6 pb-24 md:px-10 md:pt-10">
      ...
      <StarDoodle
        className="absolute top-10 left-1/2 h-8 w-8 text-accent animate-float"
        color="oklch(0.85 0.13 90)"
      />
      <SparkleDoodle className="absolute top-32 right-1/4 h-6 w-6 text-secondary opacity-60" />
      <HeartDoodle
        className="absolute bottom-32 left-[20%] h-6 w-6 text-primary opacity-70"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 items-center gap-8">
        ...
        <div className="relative hidden lg:block lg:col-span-3 h-[420px]">
          ...
          <div
            ...
            style={{ ["--r" as string]: "8deg", animationDelay: "1.2s" }}
          >
            <Polaroid
              src={sunset}
              alt="Sunset"
          ...
          <div
            className="absolute top-56 left-0 animate-float"
            style={{ ["--r" as string]: "-3deg", animationDelay: "2.4s" }}
          >
            <Polaroid
              src={flowers}
              alt="Flowers"
              caption="from a friend"
              rotate={-3}
              tape="lavender"
              className="w-44"
            />
          ...
          <HeartDoodle
            className="absolute top-24 -left-2 h-7 w-7 text-primary"
            color="oklch(0.7 0.15 12)"
          />
        </div>
        ...
        <div className="lg:col-span-6 text-center relative">
          <p className="font-hand text-2xl text-secondary mb-4">
            — a tiny love letter to your days —
          </p>
          <h1 className="font-display text-5xl md:text-6xl lg:text-7xl font-bold text-ink leading-[1.05]">
            Save the{" "}
            <span className="inline-block wobble-tilt text-primary">
              Tiny Magic
            </span>
            <br />
            of Every Day
          ...
          <p className="mt-6 font-body text-lg md:text-xl text-ink-soft max-w-xl mx-auto">
            A digital diary for your cafés, sunsets, and quiet moments — folded
            into a handmade scrapbook only you keep.
          </p>
          <div className="mt-10 flex flex-col sm:flex-row gap-4 items-center justify-center">
            <Link to="/login">
              <StickerButton>Start Your Journey</StickerButton>
            </Link>
            <a
              href="#features"
              className="font-accent text-sm font-semibold text-ink-soft underline underline-offset-4 decoration-wavy decoration-primary"
            >
              See how it folds →
            </a>
          </div>
        ...
        <div className="relative hidden lg:block lg:col-span-3 h-[420px]">
          <div
            className="absolute top-4 right-0 animate-float"
            style={{ ["--r" as string]: "6deg" }}
          >
            <Polaroid
              src={clothes}
          ...
          <div
            className="absolute top-44 right-28 animate-float"
            style={{ ["--r" as string]: "-8deg", animationDelay: "1s" }}
          >
            <Polaroid
              src={sky}
              alt="Sky"
          ...
          <div
            className="absolute top-60 right-4 animate-float"
          ...
          <StarDoodle
            className="absolute top-2 -right-2 h-7 w-7 text-secondary"
            color="oklch(0.7 0.13 295)"
          />
        </div>
  ...
  return (
    ...
    >
      <div className="text-center mb-16">
        <p className="font-accent text-xs font-bold tracking-[0.3em] text-primary uppercase">
          ...
          return (
            ...
            >
              ...
              <div className="paper-card relative torn-bottom px-7 pt-10 pb-14 min-h-[300px]">
                ...
                <p className="mt-3 font-body text-base text-ink-soft leading-relaxed">
                  {it.desc}
                </p>
                <p className="absolute bottom-6 right-6 font-hand text-lg text-primary">
                  no. 0{i + 1}
                </p>
              </div>
            </div>
    ...
    <section className="relative mx-auto max-w-7xl px-6 py-20 md:px-10">
      <div className="text-center mb-14">
        <h2 className="font-display text-4xl md:text-5xl font-bold text-ink">
          A peek at your timeline
        </h2>
        <p className="mt-3 font-hand text-2xl text-secondary">
          — scroll, smile, remember —
        </p>
      </div>
      ...
      <div className="relative">
        <svg
          className="absolute left-1/2 top-0 -translate-x-1/2 h-full w-px text-ink-soft hidden md:block"
          viewBox="0 0 2 800"
      ...
      <div className="paper-card relative torn-bottom p-6 md:p-8">
        <div className="flex items-start gap-5">
          ...
          <div className="pt-2 flex-1">
            ...
            <p className="mt-2 font-hand text-xl text-ink-soft leading-snug">
              {note}
            </p>
          </div>
        </div>
        <HeartDoodle
          className="absolute -bottom-3 right-8 h-6 w-6 text-primary"
          color="oklch(0.7 0.15 12)"
        />
      </div>
    </div>
  ...
  return (
    <section className="relative mx-auto max-w-4xl px-6 py-24 text-center">
      <div className="relative inline-block">
        <StarDoodle
          className="absolute -top-6 -left-8 h-10 w-10 text-accent"
          color="oklch(0.8 0.13 90)"
        />
        <StarDoodle
          className="absolute -top-2 -right-10 h-8 w-8 text-secondary"
          color="oklch(0.7 0.13 295)"
        />
      ...
      <div className="mt-10">
        <Link to="/login">
          <StickerButton>Get Started — it's free</StickerButton>
        </Link>
      </div>
      <HeartDoodle
        className="mx-auto mt-8 h-6 w-6 text-primary"
        color="oklch(0.7 0.15 12)"
      />
    </section>
  );
